...
#include<QToolTip>
mainWindow::mainWindow(database* db,database_utente* udb,homewindow* hw, amministratore* a) : Widget_Padre(db,udb),finestraprinc(hw),admin(a) {

    setWindowFlags ( Qt::CustomizeWindowHint | Qt::WindowTitleHint);

    tablibri=new listalibri(-1,get_model(),get_modelutenti(),admin);   //tabella con l'elenco delle opere
    tabriviste=new listariviste(-1,get_model(),get_modelutenti(),admin);
    tabutenti=new listautenti(get_model(),get_modelutenti(),admin);
    controllerLP=new C_listalibri(get_model(),get_modelutenti(),tablibri,admin,this);
    controllerRP=new C_listariviste(get_model(),get_modelutenti(),tabriviste,admin,this);
    controllerUTENTI=new C_listautenti(get_model(),get_modelutenti(),tabutenti,admin,this);
  //bottoni
    ...
    aggiungi_utente=new QPushButton("AGGIUNGI UTENTE");
  ...
  //layout
    ...
    connect(aggiungi_utente,SIGNAL(clicked()),this,SLOT(slot_inserisci_utente()));
...
void mainWindow::set_style(){
    ...
    ricerca->setPlaceholderText("Ricerca per Titolo o ID di un'opera");
    ricercautente->setPlaceholderText("Ricerca per Nome o ID di un utente");
}
...
void mainWindow::disabilita_view() {
    ...
    aggiungi_utente->setEnabled(false);
...
void mainWindow::abilita_view() {
    ...
    aggiungi_utente->setEnabled(true);
...
void mainWindow::creaLayout(){
    ...
    bottoni->addWidget(aggiungi_utente);
...
void mainWindow::slot_inserisci_utente(){
    emit show_inserisci_utente();
}
